...
/** Quantidade de vendas geradas por GT para clientes da carteira — cadastro manual em `config`. */
export interface GtsVendasOperacaoDoc {
...
export function getVendasGtOperacaoMes(
...
export async function getGtsVendasOperacao(): Promise<GtsVendasOperacaoDoc> {
...
export async function setVendasGtOperacaoMes(
  year: number,
  month: number,
  userId: string,
  total: number
  ...
  await runTransaction(db, async (trx) => {
    const snap = await trx.get(gtsVendasOperacaoRef)
    const cur = parseGtsVendasOperacaoDoc(snap.exists() ? (snap.data() as Record<string, unknown>) : undefined)
    const byUser = { ...(cur.anos[y]?.[m] ?? {}), [uid]: t }
    const meses = { ...(cur.anos[y] ?? {}), [m]: byUser }
    const anos = { ...cur.anos, [y]: meses }
    trx.set(gtsVendasOperacaoRef, { anos, atualizadoEm: serverTimestamp() }, { merge: true })
  })
}

/** Soma `adicionar` e subtrai `remover` da quantidade de vendas do GT no mês (mínimo 0). */
export async function ajustarVendasGtOperacaoMes(
  year: number,
  month: number,
  userId: string,
  adicionar: number,
  ...
  await runTransaction(db, async (trx) => {
    const snap = await trx.get(gtsVendasOperacaoRef)
    const cur = parseGtsVendasOperacaoDoc(snap.exists() ? (snap.data() as Record<string, unknown>) : undefined)
    const base = getVendasGtOperacaoMes(cur.anos, year, month, uid)
    novo = Math.max(0, base + add - rem)
    const byUser = { ...(cur.anos[y]?.[m] ?? {}), [uid]: novo }
    const meses = { ...(cur.anos[y] ?? {}), [m]: byUser }
    const anos = { ...cur.anos, [y]: meses }
